...
{
    __shared__ unsigned int base_idx[cmp_tblock_size/32+1]; // optimizing for RTM in V100 GPU.

    const int tid = threadIdx.x;
    const int bid = blockIdx.x;
    const int idx = bid * blockDim.x + tid;
    const int lane = tid & 31;  // lane in thread-block, optimizing for RTM in V100 GPU.
    const int warp = tid >> 5;  // warp in thread-block, optimizing for RTM in V100 GPU.
    ...
    // Warp-level prefix-sum (inclusive).
    ...
    // Thread-block-level prefix-sum (exclusive).
    if(warp==0)
    {
        for(int i=1; i<32; i<<=1)
        {
    ...
    // Write thread-block-level prefix-sum to global-memory.
